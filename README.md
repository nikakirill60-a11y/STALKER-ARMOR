...
- `tools/gen_assets.py` — конвертация геометрии в систему координат модели игрока
  (пивоты, зеркалирование правых рук/ног, «раздувание» от скина, чтобы не было
  z-fighting), генерация иконок предметов, лангов, рецептов и таблицы сетов.
- `src/main/java/com/stalkerarmor/` — код мода: рендер мешей через
...
Чтобы добавить новый вариант брони: положите OBJ в `armor MODEL`
